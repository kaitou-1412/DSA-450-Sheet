...
bool doubleSum(int arr[], int low, int high, int x)
{
    while(low < high)
    {
...
bool tripletSumArray(int a[], int n, int x)
{
    sort(a, a+n);
    for(int i = n-1; i >= 2; i--)
        if(doubleSum(a, 0, i-1, x - a[i]) == true) return true;
    return false;
}
...
int main()
{
    ...
    while(t--)
    {
        ...
        ip(a, n);
        if(n<3) cout << "0\n";
        else cout << tripletSumArray(a, n, x) << "\n";
    }
    return 0;
}
